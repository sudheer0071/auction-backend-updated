...
import {
  ...
  validateBidAgainstPriceLimits,
  validateBidAgainstReservePrice,
...
// Formula: Landed cost = Bid value + (Bid value * country tariff) + Sea freight * z + Land freight * z
// Where z = quantity / cartons
export const calculateLandedCost = async (amount, supplierCountry, cartons, quantity, supplierCurrency) => {
  try {
    // Convert supplier amount to GBP first
    let amountInGBP = amount;
    if (supplierCurrency && supplierCurrency !== 'GBP') {
      const currencyRate = await CurrencyRate.findOne({ code: supplierCurrency });
      if (currencyRate) {
        const rate = parseFloat(currencyRate.rate.toString());
        amountInGBP = amount / rate; // Convert to GBP (rate is how many units of currency = 1 GBP)
        console.log(`Converted ${amount} ${supplierCurrency} to ${amountInGBP} GBP (rate: ${rate})`);
      } else {
        console.warn(`No currency rate found for ${supplierCurrency}. Using amount as-is.`);
      }
    }

    // Get tariff data for supplier's country
    const countryTariff = await CountryTariff.findOne({
      country: supplierCountry,
    });

    if (!countryTariff) {
      console.warn(
        `No tariff data found for country: ${supplierCountry}. Using default landed cost = amount.`
      );
      return amountInGBP; // Fallback to just amount if no tariff data
    }

    // Convert Decimal128 to numbers
    const tariffPercentage = parseFloat(countryTariff.tariff.toString());
    const landFreightCost = countryTariff.landFreightCost
      ? parseFloat(countryTariff.landFreightCost.toString())
      : 0;
    const seaFreightCost = countryTariff.seaFreightCost
      ? parseFloat(countryTariff.seaFreightCost.toString())
      : 0;

    // Calculate z = quantity / cartons (number of containers, rounded up)
    const z = cartons > 0 ? Math.ceil(quantity / cartons) : 0;

    // Calculate landed cost: Bid value + (Bid value * tariff) + Sea freight * z + Land freight * z
    const tariffAmount = amountInGBP * (tariffPercentage / 100);
    const totalSeaFreight = seaFreightCost * z;
    const totalLandFreight = landFreightCost * z;

    const landedCost = amountInGBP + tariffAmount + totalSeaFreight + totalLandFreight;

    console.log(`Landed cost calculation for ${supplierCountry}:`);
    console.log(`  Amount (GBP): ${amountInGBP}`);
    console.log(`  Tariff (${tariffPercentage}%): ${tariffAmount}`);
    console.log(`  z (quantity/cartons): ${quantity}/${cartons} = ${z}`);
    console.log(`  Sea freight (${seaFreightCost} x ${z}): ${totalSeaFreight}`);
    console.log(`  Land freight (${landFreightCost} x ${z}): ${totalLandFreight}`);
    console.log(`  Total landed cost: ${landedCost}`);

    return landedCost;
  } catch (error) {
    console.error("Error calculating landed cost:", error);
    return amount; // Fallback to amount if calculation fails
  }
};

// Helper function to broadcast updated rankings
export const broadcastUpdatedRankings = async (
  io,
  auctionId,
  auctionSettings,
  socket = null
  ...
  try {
    ...
    // Sort by landed_cost (lowest first) - lowest landed_cost gets rank 1
    const sortedBids = allBids.sort((a, b) => {
      const aLanded =
        typeof a.landed_cost === "number"
          ? a.landed_cost
          : a.landed_cost ?? a.totalCost ?? 0;
      const bLanded =
        typeof b.landed_cost === "number"
          ? b.landed_cost
          : b.landed_cost ?? b.totalCost ?? 0;
      return aLanded - bLanded; // lowest landed_cost gets rank 1
    });

    // Create ranking data with ranks (include both for backward compatibility)
    const rankings = sortedBids.map((bid, idx) => ({
      supplierId: bid.supplier._id,
      supplierEmail: bid.supplier.email,
      bidId: bid._id,
      totalCost: bid.totalCost, // retained for existing consumers
      landed_cost: bid.landed_cost, // new primary ranking metric
      rank: idx + 1,
    ...
    if (socket) {
      ...
      console.log(
        `Sent ranking update for auction ${auctionId} to a single socket.`
      );
    } else {
      io.emit("rankingUpdate", { rankings });
      console.log(
        `Broadcasting ranking update for auction ${auctionId}:`,
        rankings
      );
    }
  } catch (error) {
  ...
  try {
    ...
    if (!auctionSettings) {
      return res.status(400).json({
        message:
          "Auction settings not found. Please configure auction settings with percentage limits before accepting bids.",
      });
    }
    ...
    if (!lot.current_price || lot.current_price <= 0) {
      return res.status(400).json({
        message:
          "Lot current price not set. Please configure a valid current_price for this lot before accepting bids.",
      });
    }
    ...
    if (!auction.invitedSupplierEmail.includes(userEmail)) {
      return res
        .status(403)
        .json({ message: "You are not invited to this auction" });
    }
    ...
    if (typeof qualificationPrice !== "number") {
      return res.status(400).json({
        message:
          "Lot qualification_price not set. Please configure a valid qualification_price for this lot before accepting bids.",
      });
    }
    const convertedQualificationPrice = await convertGBPToCurrency(
      qualificationPrice,
      currency
    );
    console.log(
      "convertedQualificationPrice .... ",
      convertedQualificationPrice
    );
    ...
    // Get cartons from EventParticipant
    const participant = await EventParticipant.findOne({
      event_id: auctionId,
      "participant.email": userEmail,
    });

    const participantCartons = participant?.cartons || 0;
    console.log(`Participant cartons for ${userEmail}: ${participantCartons}`);
    ...
    const bid = new Bid({
      ...
      bidRange: {
        min: validation.floorPrice.toFixed(2),
        max: validation.ceilingPrice.toFixed(2),
      },
      performanceScore,
    });
    ...
    try {
      console.log(
        "Creating bid history - auctionId:",
        auctionId,
        "lotId:",
        lotId,
        "supplier:",
    ...
    try {
      ...
      console.error(
        "Failed to update participant lots_entered status:",
        participantError
      );
    ...
    res
      .status(500)
      .json({ message: "Bid submission failed", error: err.message });
  }
};
...
export const updateBid = async (req, res) => {
  try {
    ...
    const { auctionId, amount, currency, fob, tax, duty, performanceScore } =
      req.body;
    ...
    if (!auctionSettings) {
      return res.status(400).json({
        message:
          "Auction settings not found. Please configure auction settings with percentage limits.",
      });
    }
    ...
    if (!lot.current_price || lot.current_price <= 0) {
      return res.status(400).json({
        message:
          "Lot current price not set. Please configure a valid current_price for this lot.",
      });
    }
    ...
    if (bid.currency !== currency) {
      ...
      try {
        ...
        const previousBidToGBP = await convertCurrencyToGBP(
          bid.amount,
          bid.currency
        );
        ...
        const gbpToNewCurrency = await convertGBPToCurrency(
          previousBidToGBP.gbpAmount,
          currency
        );
        ...
        console.log(
          `Currency conversion for comparison: ${bid.amount} ${
            bid.currency
          } = ${previousBidInNewCurrency.toFixed(6)} ${currency}`
        );
    ...
    if (bidDirection === "reverse") {
      if (!(amount < previousBidInNewCurrency)) {
        return res.status(400).json({
          message: `In reverse auction, new bid amount (${amount} ${currency}) must be less than your previous bid (${previousBidInNewCurrency.toFixed(
            2
          )} ${currency}).`,
          bidCurrency: currency,
          previousBidConverted: `${previousBidInNewCurrency.toFixed(
            2
          )} ${currency}`,
      ...
      if (!(amount > previousBidInNewCurrency)) {
        return res.status(400).json({
          message: `In forward auction, new bid amount (${amount} ${currency}) must be greater than your previous bid (${previousBidInNewCurrency.toFixed(
            2
          )} ${currency}).`,
          bidCurrency: currency,
          previousBidConverted: `${previousBidInNewCurrency.toFixed(
    ...
    try {
      console.log(
        "Validating bid update against percentage-based price limits..."
      );
    ...
    if (auction?.autoExtension === true) {
      ...
      const isHighest = allAmounts.every(
    ...
    // Get user for country and participant for cartons
    const user = await User.findById(req.user.userId);
    const participant = await EventParticipant.findOne({
      event_id: bid.auction,
      "participant.email": user.email,
    });
    const participantCartons = participant?.cartons || 0;

    // Recalculate landed cost with updated amount
    const landedCost = await calculateLandedCost(
      amount,
      user.profile.country,
      participantCartons,
    ...
    try {
      ...
      console.log("Createing bid histoary ////\\\\\/\/\/\/\/")
      await BidHistory.create({
        auction: bid.auction,
        lot: bid.lot,
        supplier: req.user.userId,
...
export const getBidHistory = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch bid history", error: err.message });
  }
};
...
export const getAuctionRanking = async (req, res) => {
  try {
    ...
    if (req.user.role === "supplier") {
      ...
      // For admin/EP, return all bids
      bids = await Bid.find({ auction: auctionId, status: "Active" }).populate(
        "lot supplier"
      );
    }

    // Sort by landed_cost (highest first now). Fallback to totalCost/0 if missing.
    const sortedBids = bids.sort((a, b) => {
      ...
      const bLanded =
        typeof b.landed_cost === "number"
          ? b.landed_cost
          : b.landed_cost ?? b.totalCost ?? 0;
      return bLanded - aLanded; // highest landed_cost rank 1
    });
    ...
    const rankedBids = sortedBids.map((bid, idx) => ({
      ...bid.toObject(),
      rank: idx + 1,
      landed_cost: bid.landed_cost, // ensure present in response
    }));
    ...
    res
      .status(500)
      .json({ message: "Failed to get auction ranking", error: err.message });
  }
};
...
export const getAuctionBidConstraints = async (req, res) => {
  try {
    ...
    if (!auctionSettings) {
      return res.status(400).json({
        message:
          "Auction settings not configured. Please set up auction settings with increment limits.",
      });
    }

    // Check if increment percentages are configured
    if (
      !auctionSettings.minimum_bid_change ||
      !auctionSettings.maximum_bid_change
    ) {
      return res.status(400).json({
        message:
          "Bid increment percentages not configured. Please set minimum_bid_change and maximum_bid_change (e.g., 0.5 for 0.5%, 10 for 10%).",
      });
    }
    ...
    if (lotId) {
      ...
      if (lot && lot.current_price) {
        const minIncrement =
          (lot.current_price * constraints.minBidPercentage) / 100;
        const maxIncrement =
          (lot.current_price * constraints.maxBidPercentage) / 100;
        ...
        constraints.validRange = `${floorPrice.toFixed(
          2
        )} - ${ceilingPrice.toFixed(2)} ${auction.default_currency || "GBP"}`;
        constraints.explanation = `For this lot: Bids must be between ${floorPrice.toFixed(
          2
        )} (current price ${lot.current_price} + ${
          constraints.minBidPercentage
        }% increment) and ${ceilingPrice.toFixed(2)} (current price ${
          lot.current_price
        } + ${constraints.maxBidPercentage}% increment) ${
          auction.default_currency || "GBP"
        }`;
      }
    }
